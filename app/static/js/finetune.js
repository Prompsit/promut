...
let draw_stacks = (stacks) => {
    if (stacks) {
        if (stacks.training)
            draw_stack(stacks, "training", document.querySelector(".training-zone"));
        if (stacks.dev)
            draw_stack(stacks, "dev", document.querySelector(".dev-zone"));
        if (stacks.test)
            draw_stack(stacks, "test", document.querySelector(".test-zone"));
...
let draw_stack = (stacks, tag, container) => {
    ...
    const pxps_am =
        ($(container).find(".area-value-max").offset().top -
            $(container).find(".area-value-min").offset().top) /
        (max_amount - min_amount);
    const threshold = min_amount * 0.15; // If the current sentence amount is min * threshold, almost there

    let stack = stacks[tag];
    $(container)
        .find(".area-value-current")
        .html(sentences_amount(stack.sentences));
    ...
    for (let corpus of stack.corpora) {
        let template = document.importNode(
            document.querySelector("#area-corpus-template").content,
            true
        );
        $(template)
            .find(".area-corpus-amount")
            .html(sentences_amount(corpus.selected_size));
        $(template)
            .find(".area-corpus-percent")
            .html(Math.round((corpus.selected_size / corpus.size) * 100));
        $(template).find(".area-corpus-name").html(corpus.name);
        $(template)
            .find(".area-corpus-delete")
            .on("click", function (e) {
                e.preventDefault();
...
$(document).ready(function () {
    ...
    function getModels() {
        ...
        formData.append("source_lang", $(".source_lang option:selected").val());
        formData.append("target_lang", $(".target_lang option:selected").val());

        $.ajax({
            url: '/finetune/get-opus-models',
            method: 'POST',
            contentType: false,
            cache: false,
            data: formData,
            ...
            success: function (response) {
                if (!response["model_list"].length) {
                    $("#engine_name").append(`<option value="" disabled>No results found</option>`)
                } else {
                    response["model_list"].forEach((mod) => {
                        const model = mod.name;
                        const id = mod.id;
                        const modelOption = `
             <option value="${model}" data-identifier="${id}">${model}</option>`;
                        $("#engine_name").append(modelOption)
        ...
        $(el).DataTable({
            ...
            drawCallback: function (settings) {
                ...
                $(el)
                    ...
                    .each(function (i, row) {
                        if (
                            ($(row).attr("data-src") != sel_src ||
                                $(row).attr("data-trg") != sel_trg) &&
                            ($(row).attr("data-src") != sel_trg ||
                                $(row).attr("data-trg") != sel_src)
                        ) {
                            $(row).addClass("d-none");
                        }

                        for (let corpus of corpora_stacks[$(row).attr("data-corpus")]
                            .corpora) {
                        ...
                        if (
                            $(el).find(".row-corpus").length ==
                            $(el).find(".row-corpus[class*='d-none']").length
                        ) {
                            let template = document.importNode(
                                document.querySelector("#no-corpora-template").content,
                                true
                            );
                        ...
                        let max = parseInt(
                            $(row).find(".corpus-selector-add").attr("data-corpus-lines")
                        );
                        ...
                        if (new_max > 0) {
                            ...
                            if (
                                parseInt($(row).find(".corpus-selector-lines").val()) > new_max
                            ) {
                ...
                $(el)
                    ...
                    .on("click", function (e) {
                        ...
                        let selected_lines = parseInt(
                            $(this)
                                .closest(".input-group")
                                .find(".corpus-selector-lines")
                                .val()
                        );
                        ...
                        if (selected_lines) {
                            if (
                                ...
                                max_amounts[corpus_type]
                            ) {
                                selected_lines =
                                    max_amounts[corpus_type] -
                                    corpora_stacks[corpus_type].sentences;
                            }
                ...
                $("body").on("click", function (e) {
                    ...
                    if (
                        !$(el).find(".corpus-selector-add").is(e.target) &&
                        $(el).find(".corpus-selector-add").has(e.target).length === 0 &&
    ...
    $(".finetune-form").on("submit", function () {
        $(".token-alert").removeClass("d-none");

        let data = new FormData();
        $(".finetune-form input").each(function (i, el) {
            if ($(el).attr("name")) {
                data.append($(el).attr("name"), $(el).val());
            }
        });

        $(".finetune-form select").each(function (i, el) {
            if ($(el).attr("name")) {
        ...
        for (let stack in corpora_stacks) {
            for (let corpora of corpora_stacks[stack].corpora) {
                data.append(
                    stack + "[]",
                    JSON.stringify({ id: corpora.id, size: corpora.selected_size })
                );
            }
        }
    ...
    $(".finetune-form").on("reset", function () {
        setTimeout(() => {
