...
from app.models import OAuth, User, UserLanguage, LibraryEngine, Engine, Role
...
def google_logged_in(blueprint, token):
    ...
    if account_info.ok:
        ...
        query = User.query.filter_by(
            social_id=social_id, username=username, email=email
        )
        ...
        try:
            ...
            if not app.config["ENABLE_NEW_LOGINS"]:
                if (
                    app.config["WHITELIST"] is None
                    or email not in app.config["WHITELIST"]
                ):
            ...
            user = User(
                ...
                username=username,
                email=email,
                avatar_url=account_info_json["picture"],
                role=beginner_role,
            )
            db.session.add(user)
            db.session.commit()
                ...
                for line in langs_file:
                    ...
                    if line:
                        ...
                        user_language = UserLanguage(
                            code=data[0], name=data[1], user_id=user.id
                        )
        ...
        with open(
            ...
            for line in langs_file:
                ...
                if line:
                    data = line.split(",")
                    user_language = UserLanguage(
                        code=data[0], name=data[1], user_id=demo_user.id
                    )
                    db.session.add(user_language)
            db.session.commit()
    ...
    for engine_path in [x.path for x in os.scandir(preloaded_path) if x.is_dir()]:
        ...
        if len(engine_data) == 6:
            source_lang = UserLanguage.query.filter_by(
                code=engine_data[2], user_id=user_id
            ).one()
            target_lang = UserLanguage.query.filter_by(
                code=engine_data[3], user_id=user_id
            ).one()
            eng = Engine(
                name=engine_data[0],
                path=engine_data[1],
            ...
            print(
                "Could not create engine for {} - incomplete information".format(
                    engine_path
                )
            )
